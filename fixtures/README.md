...
`cgi003_prism_compose.json` — **CGI-003** witness (`prism_in_compose`) retained for other cases; compose_prism.opt exercises CGI-004 wiring (legacy negative harness; alias-ok now in cgir units).
...
# Note (M7 goldens separate carve-out): the 3 M7 .opt (compose_prism_bias etc) + fixtures/rust counterparts; golden_rust_* registered and execute/pass using on-disk files (no git commit); conformance harness uses manual transpile/contains (separate). Documented in PLAN capture.
...
# IMPL 47dced29 hygiene note (M7 final/hygiene + verif + sync): harness comment tightened for precision (bias values in 3 M7 .opt files + negative using compose_prism.opt for type wiring (CGI-004) + "goldens separate" + 4 tests matrix + Unknown); removed stub/dupe markers; core phrasing identical (prefixes/hygiene adapted per doc) + qualified captures (the four markdown files; pre-existing qualified base dirty tree: M PLAN.md +107 lines); 7 files text hygiene this-delta; full re-verify 0 drift; 0 open. Pre-existing qualified (base dirty tree); this-delta exhaustive: 7 files text hygiene (crates/optic-cli/tests/execution.rs + the four markdown files + hir + diagnostics_json); See PLAN + /tmp/grok-impl-summary-47dced29.md .
