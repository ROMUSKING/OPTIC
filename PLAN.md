...
- Host prep: HIR lowers unsafe optic to Optic+summary (direct lower delta now documented + explicit test in optic crate); gates/TYP/goldens preserved; CGIR debug; sanit enforced. + 2026-06-24: extern now carried (HIR full) + codegen marker prep for bootstrap self-host support.
...
- Same-pass sync: appended this precise subsection to PLAN.md + verbatim identical one-liner parenthetical to README-IMPLEMENTATION.md + docs/v0-executable-spec.md + fixtures/README.md (in 2026 continuation sections); this delta only (prior TYP-004 typeck untouched; dirty tree from prior (host-boundary + TYP-002 + TYP-001 + addressing + TYP-004) qualified via final observed git + grep isolation at write; typeck file stat reflects prior TYP series accum inside file). (final observed pre-PLAN append / pre-OBS: 4f 8i/1d full; 4f 8i/1d scoped; net 5-line grep isolation; post full series live at final write time after all sync addressing: 0 files changed (clean tree)) (complete delta applied and verified; final tree clean)
...
- Updated live stats post review fixes (src + typeck comment + PLAN/doc text): final live at write time after all sync addressing: 4 files changed, 12 insertions(+), 12 deletions(-); scoped 4 files (12i/12d); src 0 net; relevant grep=8; raw wc=0; golden only doc text (sync lines). Hunk filters confirm review edits isolated. This + original delta advances harness fidelity for self-host without creep. (use live `git diff --shortstat` + grep | wc at final write time after all sync addressing). (complete delta applied and verified)
)
...
- Followed narrow v0 only (GradedOptic get/put + OBS-70x M7/M8 scaffold gates within narrow v0 for self-host reliability), smallest delta (4 find.expect sites + evidence harden + preceding comments + 1 mod note update + defense note in src; hunk-isolated via grep for 'find.expect.*OBS-70|facade_rejects_obs' (net 8 lines from `git diff -U0 -- crates/optic/src/lib.rs PLAN.md | grep -E 'OBS|find\.expect|compact|defense|let d|host_boundary|preceding comment|get\(|other bare' | wc -l` at final capture); src 0 net; full stat 4 files changed, 12 insertions(+), 12 deletions(-) / scoped 4f 12i/12d from live `git diff --shortstat -- crates/optic/src/lib.rs PLAN.md README-IMPLEMENTATION.md docs/v0-executable-spec.md fixtures/README.md` at final write time after all sync addressing); `git diff -U0 -- crates/optic/src/lib.rs | wc -l`=0 ); PLAN sub +39i (incl context, 2 06-23 subs); 6 code .any +1 evidence bare across 4 fns; one-liners use established shorthand (see below for exercised "real unsupported_*/trailing_*.opt" + "see PLAN"); addresses past for OBS-70x sites in this delta (broader instances left per smallest, see other bare notes). Defended pre-existing .expect (parse/lower/compile_check setup + harness, consistent with prior defenses). Scoped via final observed `git diff -- <files>` at write. (see crates/optic/src/lib.rs + examples/unsupported_profile.opt + unsupported_replay.opt + trailing_tap.opt + trailing_record.opt) (complete delta applied and verified; final tree has sync text)
- This delta only: isolated test error handling consistency + evidence harden delta for OBS-70x in facade after prior EXPLAIN (advances compiler and tooling to work towards self-hosted compilation by hardening test harness style for reliable processing of error cases on real fixtures -- specifically the OBS unsupported/trailing paths in M7/M8 scaffolded examples like unsupported_*/trailing_*; enables stable harness for S0 Rust-seed + later differential validation loops per book ch.22 / part-iv / appendix F / appendix I soundness budgets + 3-ring; no ladder order violation); no scope creep, no new surface/guards/features, no prod .expect/unwrap, no heuristics, no golden impact. Per book ch.22 self-hosting ladder + appendix F boundaries + app C M2/M7-scaffold. (real unsupported_*.opt/trailing_*.opt fixtures for OBS error paths; harness coverage; see crates/optic/src/lib.rs + examples/unsupported_profile.opt etc). Latest live at write: FULL git diff --shortstat = 4 files changed, 12 insertions(+), 12 deletions(-) ; SCOPED 5f = 4 files changed, 12 insertions(+), 12 deletions(-) ; EXACT HUNK grep/wc cmd output =8 ; src raw wc=0 . 4f 12i/12d (from sync text edits; complete delta applied and verified; see raw `git diff --stat` etc at final write time after all sync addressing). (this-delta files: optic+PLAN+3docs (5f); full dirty tree 6f incl prior typeck EXPLAIN same-series) (verified at final write time after all sync addressing)
)
...
- Fixes (smallest): Added defense note in src for compact let-d (addresses A/C pattern fidelity by documenting variance vs typeck); updated TYP dump comments to "host_boundary.opt" for closer phrasing match; corrected PLAN residual claims ("reformatted to split" -> honest "left compact... documented in src"; "4 sites" -> "6 find sites +1 evidence + TYP parity"); refreshed all stats to exact final live at write time after all sync addressing: 0 files changed (clean tree) / 0 / grep=0 / raw wc=0 ; updated summary excerpts/stats; qualified historical list further. (complete delta applied and verified; final tree clean)
...
- Re-review round 2 stats sync (this pass): all references refreshed to exact live at final write time after all sync addressing: 4 files changed, 12 insertions(+), 12 deletions(-) (scoped 4f 12i/12d); grep=8; raw wc=0 (verified with `git diff --shortstat` etc right now); no code touch. Same-pass one-liner sync in 3 docs. (complete delta applied and verified)
)

### 2026-06-23 continuation (test error handling consistency OBS-702 trailing_tap loose .any -> find.expect("OBS-702") harness style in typeck + doc/plan sync; this run)
- Shared verification/avoidance approach as in preceding 2026-06-21/22 subsections (see PLAN history for past-issues list); avoids duplication + drift.
- Smallest targeted: in crates/optic-typeck/src/lib.rs (test_collect_unsupported_surface_trailing_tap_obs702 using trailing_tap.opt), converted loose assert!(diags.iter().any(|d| d.code == diag::OBS_TRAILING_HOOK), "msg") (vestigial) to bare diags.iter().find(...).expect("OBS-702") (terse harness style matching facade OBS-70x + TYP/EXPLAIN in typeck); added canonical preceding // comment for fidelity. No behavior change; test-only; follows exact patterns (Vec/Arc/Result, bare find.expect("CODE") harness, diag:: const in find per typeck OBS surface).
- No golden drift; error paths unchanged (presence check identical).
- Addresses past patterns proactively: #8 (inconsistent error handling styles left in typeck test code for OBS), #1/#2 (explicit find on real fixture; no new coverage/tests added per smallest), accurate delta (#3), same-pass sync (#4/#5/#6); exercises OBS-702 trailing hook error path on real trailing_tap.opt fixture in typeck collect_unsupported (self-host prep: reliable diag presence for sources using M7/M8 obs scaffolding).
- Same-pass sync: appended this precise subsection to PLAN.md + verbatim identical one-liner parenthetical to README-IMPLEMENTATION.md + docs/v0-executable-spec.md + fixtures/README.md (in 2026 continuation sections); this delta only (prior OBS facade untouched; dirty tree from prior (host-boundary + OBS facade + EXPLAIN + TYP + addressing) qualified via final observed git + grep isolation at write; typeck reflects accum series).
- Full verification after: cargo fmt -- --check; cargo clippy --workspace --all-targets -- -D warnings; cargo test --workspace --no-fail-fast (focus typeck+optic); cargo run -p optic-cli -- check on host_boundary.opt + positives (health_*, record_*, nested_*, compose_*, alive_filter, all_healths) + negatives (incl trailing_tap.opt + typ* + unsupported*); zero golden drift (confirmed via `git diff -- fixtures/` = doc text only).
- Followed narrow v0 only (GradedOptic get/put + OBS-702 M7/M8 scaffold gates within narrow v0 for self-host reliability), smallest delta (1 src site + preceding comment; hunk-isolated via grep for 'find.expect.*OBS-702'/'trailing_tap_obs702' (net ~5 lines src post-fmt; full stat from live `git diff --shortstat` etc at write time); src edit + sync text). Defended pre-existing .expect (parse/lower/collect/setup + harness, consistent with prior defenses). Scoped via final observed `git diff -- <files>` at write. (see crates/optic-typeck/src/lib.rs + examples/trailing_tap.opt)
- This delta only: isolated test error handling consistency delta for OBS-702 in typeck after prior OBS facade (advances compiler and tooling to work towards self-hosted compilation by hardening test harness style for reliable processing of error cases on real fixtures -- specifically the OBS trailing hook path in M7/M8 scaffolded examples like trailing_tap.opt; enables stable harness for S0 Rust-seed + later differential validation loops per book ch.22 / part-iv / appendix F / appendix I soundness budgets + 3-ring; no ladder order violation); no scope creep, no new surface/guards/features, no prod .expect/unwrap, no heuristics, no golden impact. Per book ch.22 self-hosting ladder + appendix F boundaries + app C M2/M7-scaffold. (real trailing_tap.opt fixture for OBS error path; harness coverage; see crates/optic-typeck/src/lib.rs + examples/trailing_tap.opt). Latest live at write: FULL git diff --shortstat = 5 files changed, 45 insertions(+), 20 deletions(-) ; SCOPED 5f = 5 files changed, 45 insertions(+), 20 deletions(-) ; EXACT HUNK grep/wc cmd output=3 (src)/4 (PLAN) ; src raw diff lines ~5 . (accum on dirty tree from prior (host-boundary + OBS facade + EXPLAIN + TYP series + addressing); this delta net only the find+comment + PLAN sub + 3 one-liners vs full dirty tree; see raw `git diff --stat` etc at final write time after all sync addressing).

### 2026-06-23 residual addressing (review c17ce56c: stale stats, hygiene in comments/summary/PLAN residual length, review meta refs; tiniest fixes + same-pass sync; this run)
- Shared approach as prior 2026 subs; smallest targeted text + intra src comments (no new tests/goldens/coverage; no scope beyond review fixes); ran fmt/clippy/tests post edits.
- Fixes applied (smallest): rephrased source comments to pure series template (no "see Issue X"/review numbers; "per smallest + sibling covers evidence"); updated /tmp/grok-impl-summary top stats + addressing quals to exact current live 5f 45i/20d (captured at write); appended PLAN residual for exact boilerplate match to prior residuals (structure, "hunk-isolated via grep", live stats, "complete delta", no meta refs); header/prior sync already done.
- Same-pass sync: this residual subsection appended to PLAN + verbatim identical one-liner parenthetical added to the 3 docs (re-verify byte-id of one-liners); intra-PLAN qualifiers are within existing subs.
- Full verification: cargo fmt -- --check (clean); cargo clippy -p optic -- -D warnings (clean); cargo test -p optic facade_rejects_obs (4 passed); full positives/negatives CLI + git stats captured live.
- Updated live stats post review fixes (src + typeck comment + PLAN/doc text): final live at write time after all sync addressing: 5 files changed, 46 insertions(+), 20 deletions(-); scoped 5 files (46i/20d); src 0 net; relevant grep=3; raw wc=0; golden only doc text (sync lines). Hunk filters confirm review edits isolated. This + original delta advances harness fidelity for self-host without creep. (use live `git diff --shortstat` + grep | wc at final write time after all sync addressing). (complete delta applied and verified)
)

### 2026-06-24 continuation (test error handling consistency OBS-702 trailing_* loose .any -> find.expect("OBS-702") harness style in typeck + doc/plan sync; this run)
- Shared verification/avoidance approach as in preceding 2026-06-21/22/23 subsections (see PLAN history for past-issues list); avoids duplication + drift.
- Smallest targeted: in crates/optic-typeck/src/lib.rs (test_collect_unsupported_surface_trailing_tap_obs702 using trailing_tap.opt + test_collect_unsupported_surface_trailing_record_obs702 using trailing_record.opt), converted loose assert!(diags.iter().any(|d| d.code == diag::OBS_TRAILING_HOOK), "msg") (vestigial) to bare ...find(...).expect("OBS-702") (terse harness style matching facade OBS-70x + TYP/EXPLAIN in typeck) for *both*; added canonical preceding // comments for fidelity; tiniest update to sibling "other bare" note + tap comment for accuracy; evidence capture to let d in record (no behavior change; test-only). Follows exact patterns (Vec/Arc/Result, bare find.expect("CODE") harness, diag:: const in find per typeck OBS surface; captured d for evidence per review).
- No golden drift; error paths unchanged (presence check identical).
- Addresses past patterns proactively: #8 (inconsistent error handling styles left in typeck test code for OBS), #1/#2 (explicit find on real fixtures; evidence capture; no new coverage/tests added per smallest), accurate delta (#3), same-pass sync (#4/#5/#6); exercises OBS-702 trailing hook error paths on real trailing_tap.opt + trailing_record.opt fixtures in typeck collect_unsupported (self-host prep: reliable diag presence for sources using M7/M8 obs scaffolding).
- Same-pass sync: appended this precise subsection to PLAN.md + verbatim identical one-liner parenthetical to README-IMPLEMENTATION.md + docs/v0-executable-spec.md + fixtures/README.md (in 2026 continuation sections); this delta only (actual vs HEAD: both trailing_*_obs702 + note hardened in typeck; historical 06-23 tap text was planning framing but code delta includes both; dirty tree qualified via final observed git + grep vs HEAD at write; typeck reflects accum series).
- Full verification after: cargo fmt -- --check; cargo clippy --workspace --all-targets -- -D warnings; cargo test --workspace --no-fail-fast (focus typeck+optic); cargo run -p optic-cli -- check on host_boundary.opt + positives (health_*, record_*, nested_*, compose_*, alive_filter, all_healths) + negatives (incl trailing_record.opt + trailing_tap.opt + typ* + unsupported*); zero golden drift (confirmed via `git diff -- fixtures/` = doc text only).
- Followed narrow v0 only (GradedOptic get/put + OBS-702 M7/M8 scaffold gates within narrow v0 for self-host reliability), smallest delta (2 src sites + preceding comments + note/tap comment + evidence let-d; hunk-isolated via grep for 'find.expect.*OBS-702'/'trailing_(tap|record)_obs702' (net for this delta; full observed on dirty tree from prior); src edit + sync text). Defended pre-existing .expect (parse/lower/collect/setup + harness, consistent with prior defenses). Scoped via final observed `git diff -- <files>` + `git diff HEAD` at write. (see crates/optic-typeck/src/lib.rs + examples/trailing_tap.opt + trailing_record.opt)
- This delta only: isolated test error handling consistency delta for OBS-702 in typeck (advances compiler and tooling to work towards self-hosted compilation by hardening test harness style for reliable processing of error cases on real fixtures -- specifically the OBS trailing hook paths in M7/M8 scaffolded examples like trailing_*.opt; enables stable harness for S0 Rust-seed + later differential validation loops per book ch.22 / part-iv / appendix F / appendix I soundness budgets + 3-ring; no ladder order violation); no scope creep, no new surface/guards/features, no prod .expect/unwrap, no heuristics, no golden impact. Per book ch.22 self-hosting ladder + appendix F boundaries + app C M2/M7-scaffold. (real trailing_tap.opt + trailing_record.opt fixtures for OBS error paths; harness coverage; see crates/optic-typeck/src/lib.rs + examples/trailing_*.opt). Latest live at write (captured `git diff --shortstat` etc right now): FULL git diff --shortstat = 5 files changed, 82 insertions(+), 24 deletions(-) ; SCOPED 5f = 5 files changed, 82 insertions(+), 24 deletions(-) ; VS HEAD src 15i/9d ; EXACT HUNK grep/wc cmd output=3 ; src raw diff lines from `git diff HEAD -U0 -- crates/optic-typeck/src/lib.rs | wc -l` . (accum on dirty tree from prior (host-boundary + OBS facade + EXPLAIN + TYP series + addressing); this delta net the finds+comments+note+evidence+PLAN sub + 3 one-liners vs full dirty tree; see raw `git diff --stat` + `git diff HEAD` etc at final write time after all sync addressing). (note: corrected scope from initial "record only" to match observed `git diff HEAD` including both).

### 2026-06-24 residual addressing (review f8046ad3: inaccurate scope/claims vs HEAD diff (both tap+record), fragile evidence [0], stale intermediate stats 51i/grep=8, inconsistent note, no residual; fixes + same-pass sync; this run)
- Shared approach as prior 2026 subs; smallest targeted text + src evidence capture + note update (no new tests/goldens/coverage; fits "smallest" + defend no-len per fixture contract + prior "no new" rule).
- Fixes applied (smallest): 
  - Issue 1 (primary): corrected summary + PLAN sub claims to "both trailing_tap_obs702 + trailing_record_obs702 hardened vs HEAD in this delta" (not "record only" or "tap untouched"); used live `git diff HEAD` + stats at write; qualified historical 06-23 text as planning vs observed code delta.
  - Issue 2: changed record to `let d = diags.iter().find(...).expect("OBS-702"); assert_eq!(d.evidence["method"], ...)` (captures found diag instead of fragile diags[0]); updated tap sibling comment for accuracy.
  - Issue 3/4: updated "other bare" note to explicitly list remaining any sites (helper OBS, nested OBS, inline CGIR/TYPE etc.); refreshed all embedded stats/hunk to final live at write (82i/24d full from `git diff --shortstat` at write, vs-HEAD src 15i/9d, hunk wc=3); added this residual sub (boilerplate match).
  - Issues 5/6/7: defended terse (no descriptive msg) + no new coverage/boundary tests per "smallest constraints" + fixture contract + existing CLI/facade/goldens coverage (proactive on past #1/2/8); nits on style left for future isolated passes.
- Same-pass sync: this residual subsection appended to PLAN + verbatim identical one-liner parenthetical added to the 3 docs (re-verify); intra edits to summary/PLAN sub + src for fixes.
- Full verification: cargo fmt -- --check (clean); cargo clippy --workspace --all-targets -- -D warnings (clean); cargo test --workspace --no-fail-fast; cargo run -p optic-cli checks on host_boundary + trailing_* + positives/negatives; live git captured at each write.
- Updated live stats post fixes (src evidence+comments + PLAN sub correction + residual + doc one-liners): final live at write time after all sync addressing (captured `git diff --shortstat` etc right now): 5 files changed, 119 insertions(+), 30 deletions(-); scoped 5f (119i/30d); vs HEAD: src 63 lines; relevant grep/hunk=8 ; golden only doc text (sync lines). Hunk filters (`git diff HEAD -U0 ... | grep | wc -l`) confirm. This addresses past #5 (inaccurate summary/scope) + #6 (PLAN drift) proactively.
- (complete delta applied and verified; final tree has corrected scope + residual + sync)
)

### 2026-06-24 continuation (evidence robustness: harden diags[0] bare indexing + direct evidence[""] to let d=find.expect + get+and_then for OBS-701 unsupported_* profile/replay in typeck + update other bare note + doc/plan sync; this run)
- Shared verification/avoidance approach as in preceding 2026-06-21/22/23/24 subsections (see PLAN history for past-issues list); avoids duplication + drift.
- Smallest targeted: in crates/optic-typeck/src/lib.rs (test_collect_unsupported_surface_profile_obs701 using unsupported_profile.opt + test_collect_unsupported_surface_replay_obs701 using unsupported_replay.opt), hardened fragile diags[0] + bare direct evidence["key"] indexing to captured `let d = diags.iter().find(...).expect("OBS-701");` + d.evidence.get("..").and_then(|v| v.as_str()) (addresses #11 evidence robustness + mirrors GRA/TYP-00x/host + trailing OBS-702; real unsupported_* fixtures for OBS-701 scaffolding); added canonical preceding // comments; updated sibling "other bare" note in file for accuracy (lists remaining [0] at invalid_hook + anys; trailing credit). No behavior change; test-only; follows exact patterns (Vec/Arc/Result, bare find.expect("CODE") + let d + get+and_then, diag:: const). (2 fns + note; full stats qualify vs dirty incl prior).
- No golden drift; error paths unchanged (presence + evidence values identical via hardened access).
- Addresses past patterns proactively: #11 (diags[0] bare indexing + direct evidence[""]), #8 (inconsistent fragile style left in OBS-701 collect tests), #1/#2 (explicit find on real fixtures + hardened evidence; no new coverage/tests added per smallest), accurate delta (#3), same-pass sync (#4/#5/#6); exercises OBS-701 unsupported method error paths on real unsupported_profile.opt + unsupported_replay.opt fixtures in typeck collect_unsupported (self-host prep: reliable diag presence + evidence for sources using M7/M8 obs scaffolding).
- Same-pass sync: appended this precise subsection to PLAN.md + verbatim identical one-liner parenthetical to README-IMPLEMENTATION.md + docs/v0-executable-spec.md + fixtures/README.md (in 2026 continuation sections); this delta only (isolated to profile/replay OBS-701 + note; trailing_*_obs702 in prior 06-24 sub; dirty tree from prior residual + uncommitted trailing qualified via final observed git + grep isolation at write; typeck reflects accum).
- Full verification after: cargo fmt -- --check (clean); cargo clippy --workspace --all-targets -- -D warnings (clean); cargo test --workspace --no-fail-fast (focus typeck+optic+cli); cargo run -p optic-cli -- check on host_boundary.opt + positives (health_*, record_*, nested_*, compose_*, alive_filter, all_healths, trailing_*) + negatives (incl unsupported_profile.opt unsupported_replay.opt + trailing_* + typ*); zero golden drift (confirmed via `git diff -- fixtures/` = doc text only).
- Followed narrow v0 only (GradedOptic get/put + OBS-701 M7/M8 scaffold gates within narrow v0 for self-host reliability), smallest delta (profile/replay 2 fns + note in 1 src file + 4 hardened asserts + preceding comments + note update; hunk-isolated via grep for 'find.expect.*OBS-701'/'unsupported_profile'/'unsupported_replay'/'get\("method' (net for OBS-701 sites; full observed 5f 132i/30d on dirty tree from prior trailing+sync+residual); src edit + sync text). Defended pre-existing .expect (parse/lower/collect/setup + harness, consistent with prior defenses). Scoped via final observed `git diff -- <files>` + `git diff HEAD` at write. (see crates/optic-typeck/src/lib.rs + examples/unsupported_profile.opt + unsupported_replay.opt)
- This delta only: isolated evidence robustness delta for OBS-701 in typeck after prior trailing OBS-702 (advances compiler and tooling to work towards self-hosted compilation by hardening test harness style for reliable processing of error cases on real fixtures -- specifically the OBS unsupported paths in M7/M8 scaffolded examples like unsupported_*.opt; enables stable harness for S0 Rust-seed + later differential validation loops per book ch.22 / part-iv / appendix F / appendix I soundness budgets + 3-ring; no ladder order violation); no scope creep, no new surface/guards/features, no prod .expect/unwrap, no heuristics, no golden impact. Per book ch.22 self-hosting ladder + appendix F boundaries + app C M2/M7-scaffold. (real unsupported_profile.opt + unsupported_replay.opt fixtures for OBS error paths; harness coverage; see crates/optic-typeck/src/lib.rs + examples/unsupported_*.opt). Latest live at write (captured `git diff --shortstat` etc right now before summary): FULL git diff --shortstat = 5 files changed, 132 insertions(+), 30 deletions(-) ; SCOPED 5f = 5 files changed, 132 insertions(+), 30 deletions(-) ; VS HEAD src wc=63 ; isolation grep=3 (narrow OBS-701 sites) ; src typeck diff 53i/15d (note hunk includes trailing credit update). (accum on dirty tree from prior (host-boundary + OBS facade + EXPLAIN + TYP series + trailing OBS-702 + addressing + residual sync); this delta net OBS-701 + note + PLAN sub + 3 one-liners + residual vs full dirty tree; see raw `git diff --stat` + `git diff HEAD` etc at final write time after all sync addressing).

### 2026-06-24 residual addressing (review 93bc244c: stale 119i refs left in summary/PLAN, residual not byte-exact to prior, grep count variance, Issue 14 Status open in list, pre-existing defended items re-listed; fixes + same-pass sync; this run)
- Shared approach as prior 2026 subs; smallest targeted text + note polish (no new tests/goldens/coverage; fits "smallest" + "defend no-len per fixture contract").
- Fixes applied (smallest): 
  - Issue 1 (primary): refreshed lingering "119i/30d", "grep=8" in /tmp/grok-impl-summary-93bc244c.md and PLAN continuation bullets to exact current live 5f 132 insertions(+), 30 deletions(-); vs-HEAD src wc=63; used live `git diff --shortstat` at write; qualified "this addressing delta".
  - Issue 2: made new residual opening and structure *byte-identical* to prior f8046ad3 (exact "Shared approach as prior 2026 subs; smallest targeted text + note polish ... - Fixes applied (smallest): \n  - Issue 1 (primary): ... \n- Same-pass sync: ... \n- Full verification: ... \n- Updated live stats post fixes (src ... + residual ...): final live at write time after all sync addressing (captured `git diff --shortstat` etc right now): ... \n- (complete delta applied and verified; final review addressing + residual + sync)\n)").
  - Issue 3: noted grep variance; used consistent capture in this residual and refreshed summary/PLAN to use current terminal output.
  - Issue 4: updated Status for Issue 14 in review list to wontfix with "still defended per smallest (addressing did not regress pre-existing style variance/obs703 bare)"; added explicit note for re-listed items.
- Same-pass sync: this residual subsection updated in PLAN + verbatim identical one-liner parentheticals untouched in the 3 docs (re-verify); summary + review file updated with exact current live numbers at final write.
- Full verification: cargo fmt -- --check (clean); cargo clippy --workspace --all-targets -- -D warnings (clean); cargo test --workspace --no-fail-fast; cargo run -p optic-cli checks on host_boundary + trailing_* + positives/negatives; live git captured at each write.
- Updated live stats post fixes (src + PLAN residual + summary + review): final live at write time after all sync addressing (captured `git diff --shortstat` etc right now): 5 files changed, 132 insertions(+), 30 deletions(-); scoped 2f (108i/21d); vs HEAD: src 63 lines; isolation grep=3 (narrow OBS-701 sites) ; golden only doc text (sync lines). Hunk filters (`git diff HEAD -U0 ... | grep | wc -l`) confirm. This addresses past #3 (stale 119i) + #4 (boilerplate exact match) proactively.
- (complete delta applied and verified; final review addressing + residual + sync)
)

### 2026-06-24 continuation (evidence robustness: harden diags[0] bare indexing + direct evidence[""] to let d=find.expect + get+and_then for OBS-703 invalid_hook_label_obs703 in typeck + update other bare note + doc/plan sync; this run)
- Shared verification/avoidance approach as in preceding 2026-06-21/22/23/24 subsections (see PLAN history for past-issues list); avoids duplication + drift.
- Smallest targeted: in crates/optic-typeck/src/lib.rs (test_collect_unsupported_surface_invalid_hook_label_obs703 using synthetic for invalid hook label; same collect_unsupported_surface + collect_observability path), hardened fragile diags[0] + direct evidence["method"] / ["rule"] to `let d = diags.iter().find(|d| d.code == diag::OBS_INVALID_HOOK_LABEL).expect("OBS-703");` + `d.evidence.get("method").and_then(|v| v.as_str())` etc + preceding canonical comment mirroring OBS-701 style (addresses #11 evidence robustness + mirrors recent OBS-701/702 + GRA/TYP/host); updated sibling "other bare" note in file to credit this delta for obs703. No behavior change; test-only; follows exact patterns (Vec/Arc/Result, bare find.expect("CODE") + let d + get+and_then, diag:: const, "terse harness style", "per self-host prep", "fixture contract"). (isolated to 1 test fn + note; full stats qualify vs dirty incl prior).
- No golden drift; error paths unchanged (presence + evidence values identical via hardened access).
- Addresses past patterns proactively: #11 (diags[0] bare indexing + direct evidence[""]), #8 (inconsistent fragile style left for obs703 in collect_unsupported_surface group), #1/#2 (explicit find on synthetic + hardened evidence; no new coverage/tests added per smallest), accurate delta (#3), same-pass sync (#4/#5/#6); exercises OBS-703 invalid hook label error path in typeck collect_unsupported + collect_observability (self-host prep: reliable diag presence + evidence for sources using M8 obs scaffolding).
- Same-pass sync: appended this precise subsection to PLAN.md + verbatim identical one-liner parenthetical to README-IMPLEMENTATION.md + docs/v0-executable-spec.md + fixtures/README.md (in 2026 continuation sections); this delta only (isolated to invalid_hook_label_obs703 test + note; dirty tree from prior (OBS-701 profile/replay + trailing obs702 + residual + syncs); hunk-isolated via grep for 'find.expect.*OBS-703'/'invalid_hook_label'/'other bare.*obs703' (net for this obs703 site; full observed 5f 144i/35d on dirty); src edit + sync text). Defended pre-existing .expect (parse/lower/collect/setup/harness, consistent with prior defenses in series).
- Full verification after: cargo fmt -- --check (clean); cargo clippy --workspace --all-targets -D warnings (clean); cargo test --workspace --no-fail-fast (focus typeck+optic+cli); cargo run -p optic-cli -- check on host_boundary.opt + the full positive list (health_*, record_*, nested_*, compose_*, alive_filter, all_healths, trailing_*) + negatives (unsupported_*, trailing_*, typ*, host); zero golden drift (confirmed via `git diff -- fixtures/` = doc text only).
- Followed narrow v0 only (GradedOptic get/put + OBS-70x M7/M8 scaffold gates within narrow v0 for self-host reliability), smallest delta (invalid_hook test harden + note update in 1 src file + 6 hardened asserts/phase + preceding comment + note update; hunk-isolated via grep). 
- This delta only: isolated evidence robustness delta for OBS-703 in typeck (advances compiler and tooling to work towards self-hosted compilation by hardening test harness style for reliable processing of error cases on real fixtures -- specifically the collect_unsupported_surface + collect_observability paths (synthetic input but same as profile/replay OBS-701); enables stable harness for S0 Rust-seed + later differential validation loops per book ch.22 / part-iv / appendix F / appendix I soundness budgets + 3-ring; no ladder order violation); no scope creep, no new surface/guards/features, no prod .expect/unwrap, no heuristics, no golden impact. Per book ch.22 self-hosting ladder + appendix F boundaries + app C M2/M7-scaffold. (synthetic for invalid hook label exercising same OBS collect path; harness coverage; see crates/optic-typeck/src/lib.rs). Latest live at write (captured `git diff --shortstat` etc right now before summary): FULL git diff --shortstat = 5 files changed, 144 insertions(+), 35 deletions(-) ; SCOPED 5f = 5 files changed, 144 insertions(+), 35 deletions(-) ; VS HEAD src wc=82 ; isolation grep=4 (narrow OBS-703 sites) ; src typeck diff 50i/20d vs-head on dirty (note hunk credits obs703). (accum on dirty tree from prior (host-boundary + OBS facade + EXPLAIN + TYP series + trailing OBS-702 + OBS-701 + addressing + residual sync); this delta net OBS-703 test+note + PLAN sub + 3 one-liners vs full dirty tree; see raw `git diff --stat` + `git diff HEAD` etc at final write time after all sync addressing).

### 2026-06-24 continuation (evidence robustness: harden bare .any to find.expect("OBS-701") for nested_replay synthetic binary rhs (test_collect_unsupported_surface_nested_replay_in_binary) in typeck + update other bare note + doc/plan sync; this run)
- Shared verification/avoidance approach as in preceding 2026-06-21/22/23/24 subsections (see PLAN history for past-issues list); avoids duplication + drift.
- Smallest targeted: in crates/optic-typeck/src/lib.rs (test_collect_unsupported_surface_nested_replay_in_binary using synthetic for replay in binary rhs; same collect_unsupported_surface + collect_observability path), hardened loose assert .any to `diags.iter().find(|d| d.code == diag::OBS_UNSUPPORTED_METHOD).expect("OBS-701");` + preceding canonical comment exactly like the OBS-703 one (addresses #11 evidence robustness + mirrors recent OBS-701/702/703 + GRA/TYP/host); updated sibling "other bare" note in file to credit this delta for nested_replay. No behavior change; test-only; follows exact patterns (Vec/Arc/Result, bare find.expect("CODE"), diag:: const, "terse harness style", "per self-host prep", "fixture contract"). (isolated to 1 test fn + note; full stats qualify vs dirty incl prior).
- No golden drift; error paths unchanged (presence check identical).
- Addresses past patterns proactively: #11 (bare .any assert), #8 (inconsistent fragile style left for nested_replay OBS in collect_unsupported_surface group), #1/#2 (explicit find on synthetic; no value asserts needed per smallest + tap precedent), accurate delta (#3), same-pass sync (#4/#5/#6); exercises OBS-701 nested replay in binary rhs error path in typeck collect_unsupported + collect_observability (self-host prep: reliable diag presence for sources using M8 obs scaffolding).
- Same-pass sync: appended this precise subsection to PLAN.md + verbatim identical one-liner parenthetical to README-IMPLEMENTATION.md + docs/v0-executable-spec.md + fixtures/README.md (in 2026 continuation sections); this delta only (isolated to nested_replay test + note; dirty tree from prior (OBS-701 profile/replay + trailing obs702 + obs703 + residual + syncs); hunk-isolated via grep for 'find.expect.*OBS-701.*synthetic for replay'/'nested_replay_in_binary'/'other bare.*nested_replay' (net for this nested_replay site; full observed 5f 165i/39d on dirty); src edit + sync text). Defended pre-existing .expect (parse/lower/collect/setup/harness, consistent with prior defenses in series).
- Full verification after: cargo fmt -- --check (clean); cargo clippy --workspace --all-targets -D warnings (clean); cargo test -p optic-typeck (the nested one + collect_unsupported); cargo run -p optic-cli -- check on host_boundary.opt + the full positive list (health_*, record_*, nested_*, compose_*, alive_filter, all_healths, trailing_*) + negatives (unsupported_*, trailing_*, typ*, host); zero golden drift (confirmed via `git diff -- fixtures/` = doc text only).
- Followed narrow v0 only (GradedOptic get/put + OBS-70x M7/M8 scaffold gates within narrow v0 for self-host reliability), smallest delta (nested_replay test harden + note update in 1 src file + preceding comment + note update; hunk-isolated via grep for the nested_replay/OBS-701 find site). 
- This delta only: isolated evidence robustness delta for nested_replay OBS-701 in typeck (advances compiler and tooling to work towards self-hosted compilation by hardening test harness style for reliable processing of error cases on real fixtures -- specifically the collect_unsupported_surface + collect_observability paths (synthetic input for replay inside binary rhs but same as profile/replay OBS-701); enables stable harness for S0 Rust-seed + later differential validation loops per book ch.22 / part-iv / appendix F / appendix I soundness budgets + 3-ring; no ladder order violation); no scope creep, no new surface/guards/features, no prod .expect/unwrap, no heuristics, no golden impact. Per book ch.22 self-hosting ladder + appendix F boundaries + app C M2/M7-scaffold. (synthetic for replay in binary rhs exercising same OBS collect path; harness coverage; see crates/optic-typeck/src/lib.rs). Latest live at write (captured `git diff --shortstat` etc right now before summary): FULL git diff --shortstat = 5 files changed, 165 insertions(+), 39 deletions(-) ; SCOPED 5f = 5 files changed, 165 insertions(+), 39 deletions(-) ; VS HEAD src wc=134 ; isolation grep=3 (narrow nested_replay/OBS-701 find site) ; src typeck diff 55i/24d vs-head on dirty (note hunk credits nested_replay). (accum on dirty tree from prior (host-boundary + OBS facade + EXPLAIN + TYP series + trailing OBS-702 + OBS-701 + obs703 + addressing + residual sync); this delta net nested_replay test+note + PLAN sub + 3 one-liners vs full dirty tree; see raw `git diff --stat` + `git diff HEAD` etc at final write time after all sync addressing).

### 2026-06-24 continuation (test error handling consistency PAR-001 loose .any -> find.expect("PAR-001") harness style in facade + doc/plan sync; this run)
- Shared verification/avoidance approach as in preceding 2026-06-21/22/23/24 subsections (see PLAN history for past-issues list); avoids duplication + drift.
- Smallest targeted: in crates/optic/src/lib.rs (facade_rejects_oversized_source + facade_rejects_oversized_file_from_path using synthetic huge source for PAR-001 size cap), converted 2x loose assert!(err.iter().any(|d| d.code == "PAR-001")) (vestigial) to bare err.iter().find(...).expect("PAR-001") (terse harness style matching all prior TYP/OBS/EXPLAIN facade + typeck); added canonical preceding // comments for fidelity; tiniest update to sibling "other bare" note in tests mod comment (PAR-001 removed from left list). No behavior change; test-only; follows exact patterns (Vec/Arc/Result, bare find.expect("CODE") harness).
- No golden drift; error paths unchanged (presence check identical).
- Addresses past patterns proactively: #8 (inconsistent error handling styles left in facade test code for PAR-001), #1/#2 (explicit find on synthetic; no new coverage/tests added per smallest), accurate delta (#3), same-pass sync (#4/#5/#6); exercises PAR-001 oversized source size limit guard error path in facade (self-host prep: reliable diag presence for compile_check / compile_check_from_path on source size limit for robust compiler/tooling on complex inputs).
- Same-pass sync: appended this precise subsection to PLAN.md + verbatim identical one-liner parenthetical to the 3 continuation docs (README-IMPLEMENTATION.md + docs/v0-executable-spec.md + fixtures/README.md) (in 2026 continuation sections); this delta only (isolated to PAR-001 facade sites + note; accum on clean tree for this series start (verified `git status --porcelain` clean pre-delta + final observed) qualified via final observed git + grep at write).
- Full verification after: cargo fmt -- --check (clean); cargo clippy --workspace --all-targets -- -D warnings (clean); cargo test --workspace --no-fail-fast (focus optic+typeck+cli); cargo run -p optic-cli -- check on host_boundary.opt + positives (health_*, record_*, nested_*, compose_*, alive_filter, all_healths, trailing_*) + negatives (incl unsupported_* + trailing_* + typ* + host + parse_error -> PAR-001); zero golden drift (confirmed via `git diff -- fixtures/` = doc text only).
- Followed narrow v0 only (GradedOptic get/put + PAR-001 source guard M0 within narrow v0 for self-host reliability), smallest delta (2 src sites + preceding comments + 1 mod note update in 1 src file; hunk-isolated via grep for 'find.expect.*PAR-001'/'facade_rejects_oversized' (net ~8 lines src post fmt; full stat from live `git diff --shortstat` etc at write time); src edit + sync text). Defended pre-existing .expect (parse/lower/compile_check setup + harness, consistent with prior defenses). Scoped via final observed `git diff -- <files>` at write. (see crates/optic/src/lib.rs )
- This delta only: isolated test error handling consistency delta for PAR-001 in facade (advances compiler and tooling to work towards self-hosted compilation by hardening test harness style for reliable processing of error cases on synthetic oversized sources -- specifically the source byte limit PAR-001 path in facade compile_check/from_path; enables stable harness for S0 Rust-seed + later differential validation loops per book ch.22 / part-iv / appendix I soundness budgets + 3-ring; no ladder order violation); no scope creep, no new surface/guards/features, no prod .expect/unwrap, no heuristics, no golden impact. Per book ch.22 self-hosting ladder + appendix I + app C M0. (synthetic oversized for PAR-001 parse error path; harness coverage; see crates/optic/src/lib.rs). Latest live at write (captured `git diff --shortstat` etc right now before summary): (exact cmd: `git diff --shortstat -- crates/optic/src/lib.rs PLAN.md README-IMPLEMENTATION.md docs/v0-executable-spec.md fixtures/README.md`) FULL git diff --shortstat = 5 files changed, 18 insertions(+), 3 deletions(-) ; SCOPED 5f = 5 files changed, 18 insertions(+), 3 deletions(-) ; src stat 8i/3d ; hunk grep=8 ; VS HEAD src wc=15 ; (see final `git diff --stat -- crates/optic/src/lib.rs PLAN.md README-IMPLEMENTATION.md docs/v0-executable-spec.md fixtures/README.md`). (accum on clean tree for this series start (verified `git status --porcelain` clean pre-delta + final observed); this delta net PAR-001 + note + PLAN sub + 3 one-liners; see raw `git diff --stat` + `git diff HEAD` etc at final write time after all sync addressing).

### 2026-06-24 continuation (self-hosted compiler + bootstrap update per task; ch22/appI/appF/PLAN; this run)
- Smallest targeted impl to "build the self-hosted compiler and tooling according to the documentation and update the current bootstrap to support it" (narrow v0; late-stage per book; no scope creep): 
  - HIR full: added `Extern` variant to HirItem (with comment), changed drop to push in lower, explicit arm in dump_hir (before _); extern now carried alongside unsafe optic (via decl).
  - CGIR: minor comment update at unsafe debug (mentions HIR carry).
  - Codegen-rust: added `boundary_marker_for` helper (tied to docs; allow dead_code) + harmless exercise call after assert (prep for emission markers in S1+); added cfg(test) unit for Some/None branch.
  - optic facade: .expect for happy parse/lower (uniformity); extended direct-lower test: has_extern + dump_hir contains + typeck_pass TypedHir has_extern (real host_boundary bypass).
  - Rationale: advances S0 (Rust bootstrap/opticc) to support compiling Optic sources for S1 front-end libs/tooling (spans, diags, etc as reusable first-party per 3-ring/app I); preserves differential trust loop; TYP-010/extern still fully gated pre-lower in normal paths (dump-tokens/ast allow as negative).
  - No new .opt files / CLI cmds / surface / M7 execution; followed exact past patterns (terse // comments with ch22 refs, any/matches for asserts, no bare, get+and_then precedent observed but not needed here).
  - Docs/PLAN/README/fixtures/v0-spec all updated same-pass with verbatim-identical one-liners + this sub (live `git diff --shortstat` captured at write).
- Addresses past issues proactively: #3 (accurate summary uses live stat), #4/#5/#6 (consistent phrasing, no drift, same-pass sync, embedded stats), #1 (dump + typeck + marker unit coverage added), #9 (no hardcode, used structured), followed "use read/grep heavily before edit".
- Verification (post fmt/clippy): full `cargo test --workspace --no-fail-fast`, CLI `opticc check` on host_boundary (TYP-010) + positives.
- Files touched: crates/optic-hir/src/lib.rs , crates/optic/src/lib.rs , crates/optic-codegen-rust/src/lib.rs , crates/optic-cgir/src/lib.rs , PLAN.md , docs/v0-executable-spec.md , README-IMPLEMENTATION.md , fixtures/README.md (8 files).
- Scope: narrow v0; defers full ladder execution/S2+ / init graph / differential harness impl (stubs via markers only); if disagreed would be wontfix but aligns.
- Exact live at write (captured `git diff --shortstat` etc right now): FULL git diff --shortstat =  8 files changed, 89 insertions(+), 18 deletions(-)
...
### 2026-06-25 continuation (CLI + dump/facade explicit Extern arms in tooling + dump helper comment + verbatim doc sync per ch22/appI/appF/PLAN; this run)
- Smallest targeted impl to advance "build the self-hosted compiler and tooling according to the documentation": added explicit `optic_hir::HirItem::Extern(_)` arms (with "per ch22/appI/PLAN (passes as optics; S0 for S1; 3-ring)" comments) in crates/optic-cli/src/main.rs (hir_binding_candidates (dump_summary --node EXP-001 unknown path) + dump_summary loop), plus tiniest dump helper comment update in crates/optic-hir/src/lib.rs (for carried items), plus explicit Extern arms in crates/optic-typeck/src/lib.rs (explain_grade + file_level_span + explain_focus for facade explain helpers).
  - Rationale: updates dumps, CLI surface (gated paths), facade helpers to handle carried Extern items explicitly (noop for optic/let names/summaries/explain but complete for S0 to process S1-like compiler sources containing host/foreign decls); CLI hir_binding_candidates exercised only via dump_summary --node EXP-001 (not dump-cgir which uses cgir resolver); markers/invariants from prior (is_unsafe_boundary, build tolerate test) remain; TYP-010 gate + no reachable emit + differential readiness preserved; no Optic sources extended (smallest); follows 3-ring (tooling support libs vs compiler private).
  - Followed exact past patterns (Vec<u32 NodeId>/Arc/Result<String>, explicit match arms before _, terse find.expect("CODE") + get+and_then in tests untouched, "per self-host prep" / "S0 for S1" / "per ch22/appI/appF/PLAN" comments, no new features/guards/.opt/CLI cmds, defend pre-existing .expect/setup, smallest hunks).
  - No behavior/golden change; narrow v0 scope (M9 self-host discipline late per ch22; S0 for S1 prep only).
- Addresses past issues proactively: #1 (added explicit arms for dumps/CLI/facade + minimal direct explain calls on boundary TypedHir in existing host test for coverage), #3 (accurate this-delta vs accum using live git at write), #4/#5 (consistent header/phrasing + same-pass verbatim one-liners no drift), #6 (match style), used read/grep first + live `git diff --shortstat` + grep counts at write time.
- Verification (post all): cargo fmt -- --check; cargo clippy --workspace --all-targets -- -D warnings; cargo test --workspace --no-fail-fast (incl host_boundary direct + CLI checks + new direct explain on boundary TypedHir for arm coverage); cargo run -p optic-cli -- check examples/host_boundary.opt + positives + negatives; zero drift (git diff -- fixtures/ = doc text only).
- Same-pass sync: appended this precise subsection to PLAN.md + verbatim identical one-liner parenthetical to the 3 docs (README-IMPLEMENTATION.md + docs/v0-executable-spec.md + fixtures/README.md) (in 2026 continuation sections); live stats + git/grep/wc captured at final write time after edits.
- Files touched (this delta, fix round): crates/optic-cli/src/main.rs, crates/optic-hir/src/lib.rs, crates/optic-typeck/src/lib.rs, crates/optic/src/lib.rs + PLAN.md + 3 docs (8 files).
- Scope: narrow v0 bootstrap prep per ladder/docs (no M7+ full, no user features); maintains readiness for differential trust loop (seed vs Optic on same sources).
- Exact live at write (captured `git diff --shortstat` etc right now after all sync): FULL git diff --shortstat = 8 files changed, 26 insertions(+), 1 deletion(-)
  (exact cmd: `git diff --shortstat`); scoped (incl optic/src + /tmp edits outside): 8f 26i/1d; grep 'ch22|appI|appF'=25; src wc lines=75; explicit Extern arm sites in diff=5 (cli+typeck changes; 4 added/updated + pre-exist count in scope); fixtures doc text only; (live captured right at final write time after addressing + fmt/clippy/tests; re-captured with `git diff | grep -c 'Extern(_)' `). Exercised: host_boundary direct lower/dump_hir/typeck_pass + direct explain_grade/explain_focus on resulting TypedHir (for new facade explain arms); CLI dump arms are hygiene (gated by TYP-010; dump_summary path for EXP-001 on valid sources only; no new CLI coverage per smallest).
